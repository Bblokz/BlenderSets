bl_info = {
    ...
    "description": "Remap UVs of selected material slots into a texture atlas layout",
    "category": "UV",
}
    ...
    def mat_slot_items(self, context):
        ...
        if obj and obj.type == 'MESH':
            for i, slot in enumerate(obj.material_slots):
                items.append((str(i), slot.name or f"Slot {i}", f"Material slot {i}"))
        return items
    ...
    def pos_slot_items(self, context):
        ...
        if size == 'SEVEN':
            return [
                ...
                ('BR_TL', 'BR: Top Left', ''),
                ('BR_TR', 'BR: Top Right', ''),
                ('BR_BL', 'BR: Bottom Left', ''),
                ('BR_BR', 'BR: Bottom Right', ''),
            ]
        if size == 'TEN':
            return [
                ('TL', 'Top Left', ''),
                ...
                ('BL_TL', 'BL: Top Left', ''),
                ('BL_TR', 'BL: Top Right', ''),
                ('BL_BL', 'BL: Bottom Left', ''),
                ('BL_BR', 'BL: Bottom Right', ''),
                ('BR_TL', 'BR: Top Left', ''),
                ('BR_TR', 'BR: Top Right', ''),
                ('BR_BL', 'BR: Bottom Left', ''),
                ('BR_BR', 'BR: Bottom Right', ''),
...
class OBJECT_OT_uv_remap(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        counts = {'FOUR': 4, 'SEVEN': 7, 'TEN': 10, 'SIXTEEN': 16}
        mapping = {
            'TL': ((0, 0.5), 0.5), 'TR': ((0.5, 0.5), 0.5),
            'BL': ((0, 0), 0.5), 'BR': ((0.5, 0), 0.5),
            'BR_TL': ((0.5, 0.25), 0.25), 'BR_TR': ((0.75, 0.25), 0.25),
            'BR_BL': ((0.5, 0), 0.25), 'BR_BR': ((0.75, 0), 0.25),
            'BL_TL': ((0, 0.25), 0.25), 'BL_TR': ((0.25, 0.25), 0.25),
            'BL_BL': ((0, 0), 0.25), 'BL_BR': ((0.25, 0), 0.25),
        }
        for r, y in enumerate((0.75, 0.5, 0.25, 0.0), start=1):
            for c, x in enumerate((0, 0.25, 0.5, 0.75), start=1):
                mapping[f"R{r}C{c}"] = ((x, y), 0.25)
        n = counts[p.atlas_size]
        for i in range(1, n + 1):
            mid = getattr(p, f"slot{i}_mat")
            if mid == 'NONE':
                continue
            try:
                idx = int(mid)
                if idx < 0 or idx >= len(obj.material_slots):
                    self.report({'WARNING'}, f"Slot {i}: invalid material index {idx}")
                    continue
                pos_key = getattr(p, f"slot{i}_pos")
                off, sc = mapping[pos_key]
                ...
                self.report({'WARNING'}, f"Slot {i}: invalid position, skipping")
            except Exception as e:
                self.report({'WARNING'}, f"Slot {i} skipped: {e}")
        mesh.update()
        self.report({'INFO'}, "UV remapping complete")
        return {'FINISHED'}
...
class OBJECT_OT_uv_clear_materials(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, "Cleared material selections")
        return {'FINISHED'}
...
class OBJECT_OT_uv_clear_uv(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if size == 'FOUR':
            ...
            defaults = {1:'TL', 2:'TR', 3:'BL_TL', 4:'BL_TR', 5:'BL_BL', 6:'BL_BR',
                        7:'BR_TL', 8:'BR_TR', 9:'BR_BL', 10:'BR_BR'}
        else:
            defaults = {i: f"R{((i-1)//4)+1}C{((i-1)%4)+1}" for i in range(1, 17)}
        for i, pos in defaults.items():
            setattr(p, f"slot{i}_pos", pos)
        self.report({'INFO'}, "Reset UV slot positions to defaults")
        return {'FINISHED'}
